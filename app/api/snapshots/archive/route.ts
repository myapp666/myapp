...
// 批量归档 / 取消归档。
// Body: { ids: number[], archived: boolean }
...
export async function POST(request: NextRequest) {
  ...
  if (!Array.isArray(body.ids) || typeof body.archived !== 'boolean') {
    return NextResponse.json(
      { error: 'ids 必须是 number[]，archived 必须是 boolean' },
      { status: 400 },
    );
  }
  ...
  // 用 updateMany + userId 条件双重保险：传过来的 id 即使有别人的，也只会更新属于自己的
