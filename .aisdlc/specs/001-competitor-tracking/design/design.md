...
- **In / Out**：In = `.env` 竞对 URL 配置 + APScheduler 定时采集 + Claude Haiku 4.5 语义解读 + JSON 文件历史存储 + Next.js/Vercel 前端展示；Out = 登录/鉴权、实时告警、社媒抓取、多角色权限
- **推荐方案**：Python 后端（APScheduler + FastAPI + JSON 文件存储）独立运行，Next.js 前端部署到 Vercel，通过 REST API 连接；同一 GitHub 仓库，`backend/` + `frontend/` 双子目录结构
- **关键取舍**：Vercel 不支持常驻 Python 进程，故前后端必须网络分离（REST API）；JSON 文件替代 SQLite，零依赖、无需 schema 迁移，用 filelock 保证并发安全，数据量大时可直接迁移至 SQLite/PostgreSQL
...
- **不变量**：
  ...
  - JSON 文件仅 Python 后端写入，前端只通过 REST API 读取（规则-5）
...
- **关键交互**：运维编辑 `.env` → 重启后端 → APScheduler 按间隔采集网站 → 调用 LLM 解读 → 写入 JSON 文件 → Next.js 前端查询 FastAPI → 展示历史记录
- **关键约束**：Vercel 不支持常驻 Python 进程；JSON 文件不支持跨机器访问；无鉴权（内部访问）
...
| 容器 | 职责 | 技术选型 | 运行位置 |
|---|---|---|---|
| Python 后端 | 定时采集 + AI 解读 + REST API | Python 3.11+, APScheduler 3.x, FastAPI, filelock | 本地/VPS（独立进程） |
| Next.js 前端 | 历史记录查阅 UI | Next.js 14+, React, Vercel | Vercel（GitHub push 自动部署） |
| JSON 文件存储 | 历史快照与 AI 解读存储 | JSON 文件（每 URL 一个文件，filelock 保证写安全） | 与 Python 后端同机文件系统 |

**关键数据流**：
1. APScheduler 触发 → scraper 抓取 HTML → diff 与上次快照比较 → 有变化则调用 LLM → filelock 写入 JSON 文件
...
| `storage.py` | JSON 文件读写；每 URL 对应一个 `data/{url_slug}.json`，内含 records 数组；filelock 保证并发写安全；按 `crawled_at` 倒序排列 | `filelock`, `json`, `pathlib` |
| `api.py` | FastAPI 路由；CORS 配置（允许 Vercel 域名）；只读接口 | `fastapi`, `storage.py` |
...
scraper 抓取 HTML
  ↓（与上次快照比较）
无变化 → 不写文件，不调用 LLM（规则-2）
有变化 → 调用 LLM
         ↓
    LLM 成功 → filelock 写入 JSON 完整记录（change_type/summary/importance 非空）
    LLM 失败 → filelock 写入 JSON 记录，summary = "[AI 解读失败，原因: {error}]"（规则-3）
```

**错误处理与幂等策略**：
- 采集超时（>30s）：tenacity 指数退避，最多 3 次重试，耗尽后写日志，不中断其他 URL 同批次采集
- LLM 调用失败：降级写入 JSON（保留 HTML 快照），不抛出导致任务中断
...
| D3 | 数据存储 | JSON 文件 + filelock | 零依赖（无需数据库进程/驱动/schema 迁移）；每 URL 一个文件隔离写锁；MVP 数据量小（数百条/URL）完全够用 | 数据量大（>1 万条/URL）或需结构化查询 → 迁移 SQLite/PostgreSQL |
...
- **迁移/回滚**：JSON 文件即数据（`backend/data/{url_slug}.json`）；无 schema 迁移；回滚方式：还原 JSON 文件备份；升级至 SQLite/PostgreSQL 时，写一次性迁移脚本读 JSON 写 DB
...
| Python 后端 REST API（新建）| `GET /api/urls`、`GET /api/snapshots`（新建，无历史版本） | JSON 文件 schema（新建） | **新增**：全新服务，无破坏性变更 |
...
- **数据口径影响**：新建 JSON 文件结构（`backend/data/{url_slug}.json`），无现有数据口径变更；`change_type`/`importance` 为 LLM 自由输出字符串（MVP 阶段不强校验枚举，后续 v2 可收紧）
- **运行与运维影响**：
  - Python 后端需独立部署（本地/VPS），运维需维护进程存活（建议 systemd/supervisor）
  - `backend/data/` 目录下 JSON 文件需定期备份（简单 `cp` 或 `rsync` 即可）
  - Anthropic API Key 额度需监控，避免超额中断
  - 日志是主要排查手段（无 APM/监控），建议结构化日志写入文件
- **迁移/回滚**：
  - 新增服务不影响现有 landing page，可独立回滚
  - JSON → SQLite/PostgreSQL 迁移：写一次性脚本读取 `data/*.json` 写入目标 DB；无需停现有服务
...
- [`design/research.md`](.aisdlc/specs/001-competitor-tracking/design/research.md)：T1（LLM）、T2（APScheduler）、T3（采集策略）、T4（原 SQLite WAL，已更新为 JSON 文件存储）、T5（REST API）、T6（Vercel+GitHub 部署）
...
- 2026-07-07：D3 存储决策从 SQLite WAL + Alembic 更新为 JSON 文件 + filelock；相关联动更新（容器清单、组件表、状态流转、对外承诺、影响分析、ADR 建议）
